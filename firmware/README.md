...
| `agg` | **요약 모드** | 0/1 | 1=측정구간당 **∫I·dt 후반부 선형회귀 기울기 1샘플**만 저장/전송 (raw 폐기). CYCLE/TIMED 전용, 기본 0 |
...
{"st":"idle|run|rest","mode":0,"rate":10,"cyc":3,"range":1,"pend":0,"buf":12,"gap":0,
 "vbat":3720,"chg":1,"qi":1,"lb":0,"rf":1000000,"os":64,"fpend":0,"agg":0}   // 상태 (1초 주기) — vbat 이후가 확장 필드
```
...
- **저장**: BLE 미연결 동안 샘플이 내부 플래시 파티션(96KB)에 적층 —
  **~6,100샘플** = cycle(5초 측정/5분) raw 기준 **~10시간**, 1 Hz 연속 기준 ~1.7시간.
- **요약 모드(`agg:1`) 권장**: 사이클당 1샘플이 되어 같은 96KB 로 **~21일**.
  기울기 값은 후반 2.5초(25샘플 회귀)의 평균 효과로 끝값 대비 노이즈 ~5× 감소.
...
- **측정 중엔 재생 일시정지**: 백로그 재생은 IDLE/사이클 휴지기에만 —
  코인셀 IR 전압새그(라디오 버스트+측정 부하 중첩)와 라디오→아날로그 노이즈
  침투를 피하기 위함. 사이클 모드는 휴지기가 대부분이라 동기화 시간 충분.
