...
| RF | 내부 RTIA | 외부 2 MΩ (R9 — 교체 가능) → **`rf` 런타임 설정, 기본값 2M** |
...
| `src/flog.c` | **플래시 오프라인 백로그** — 폰 없이 측정, 재연결 시 자동 동기화 (96KB, ~6,100샘플) |
...
{"st":"idle|run|rest","mode":0,"rate":10,"cyc":3,"range":1,"pend":0,"buf":12,"gap":0,
 "vbat":3720,"chg":1,"qi":1,"lb":0,"rf":1000000,"os":64,"fpend":0}   // 상태 (1초 주기) — vbat 이후가 확장 필드
```

`chg` = 실제 충전 중(MCP73832 STAT) · `qi` = 충전패드 위(Qi 5V 수신, AIN3 분압 >2 V).
둘을 조합하면 앱에서 3상태 구분: 패드 밖(`qi:0`) / 충전 중(`qi:1,chg:1`) / 완충(`qi:1,chg:0`).
`fpend` = 플래시 백로그(미동기화 샘플 수) — 0 이 되면 동기화 완료.

## 웨어러블 오프라인 측정 (플래시 백로그)

폰 없이 착용한 채 측정하고, 나중에 앱을 열면 자동으로 밀린 데이터가 동기화된다.

- **저장**: BLE 미연결 동안 샘플이 내부 플래시 파티션(96KB)에 적층 —
  **~6,100샘플** = cycle(5초 측정/5분) 기준 **~10시간**, 1 Hz 연속 기준 ~1.7시간.
- **동기화**: 재연결하면 백로그를 오래된 것부터 기존 `{"d":...}`+ack 스트림으로
  재생 → **앱 무수정**. 배치 단위 stop-and-wait 라 ack 유실 시 재전송(중복은 앱이
  seq 로 제거). 백로그가 다 빠진 뒤 라이브 스트림 재개 (시간순 보존).
- **재부팅 생존**: 페이지 헤더 + 레코드 CRC 로 부팅 시 백로그 복원 (3.0 V
  System OFF 후 충전해서 켜도 데이터 유지).
- **가득 차면**: 가장 오래된 페이지부터 덮어쓰고 status `gap:1` 로 보고.
- 마모 수명: cycle duty 기준 페이지당 지우기 주기가 길어 (~10 h/링 순환) 연간 수천
  사이클 — 1만 회 한계 대비 여유 큼. 고속 연속(10 Hz+) 장기 로깅은 외부 SPI 플래시
  확장 권장 (회로도에 자리만 예약해두는 옵션 있음).

## 배터리 정책 (LIR2032 무보호 셀)

- 10분마다 VBAT(AIN2, 1M:1M 분압) 측정 (부팅 직후 1회 즉시 측정) → status 의 `vbat`[mV].
...
- **flog 파티션**: 0x62000~0x7A000 (96KB). 빌드 로그의 FLASH 사용량이 **392KB(0x62000)를 넘으면** app.overlay 의 파티션 시작을 뒤로 밀 것.
