...
- **바이어스**: 하드웨어 고정. AVDD 급전 GPIO(P0.07) HIGH → REF35102(1.024 V) + LPV802 급전(~2 µA),
  50 ms 안정화 후 샘플링. 정지 시 LOW → 소모 0 + CE 차단(전극 분극 방지).
  (회로도 요건: DVDD→AVDD 상시 연결(R7 등) 제거, AVDD 는 P0.07 이 단독 급전)
...
| AVDD 급전 (AFE 전원) | P0.07 | high-drive 출력 — DVDD→AVDD 상시 연결 없어야 함 |
