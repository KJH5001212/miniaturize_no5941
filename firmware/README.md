...
| `src/meas.c` | SAADC 엔진 (차동 AIN1−AIN0, 게인 3단, HW 오버샘플) + AFE 전원 + VBAT/충전 |
...
| `app.overlay` | RevC 핀맵 · DC/DC 모드 · SAADC 활성 |
| `prj.conf` | BLE(NUS)/ADC/FPU/RTT/settings/LFRC 설정 |

## 측정 원리

- **바이어스**: 하드웨어 고정. AFE_PWR(P0.06) HIGH → REF35102(1.024 V) + LPV802 급전(~2 µA),
  50 ms 안정화 후 샘플링. 정지 시 LOW → 소모 0 + CE 차단(전극 분극 방지).
- **TIA**: `V_TIA = 1.024 V + I×RF`. SAADC **차동**(AIN1=TIA_OUT, AIN0=1.024 V)이라
...
{"st":"idle|run|rest","mode":0,"rate":10,"cyc":3,"range":1,"pend":0,"buf":12,"gap":0,
 "vbat":3720,"chg":1,"lb":0,"rf":1000000,"os":64}   // 상태 (1초 주기) — vbat 이후가 확장 필드
```
...
- 보드 타깃은 DK 지만 `app.overlay` 가 RevC 핀맵/전원으로 전부 재정의한다.
- 32.768 kHz 크리스탈 없음 → `CONFIG_CLOCK_CONTROL_NRF_K32SRC_RC=y` (이미 설정).
- DC/DC 인덕터 실장 → DCDC 모드 (overlay). LDO 로 폴백하려면 overlay 의 `&reg` 블록 제거.
- 플래시: J-Link + SWD (J2 납땜 패드: 3V3/SWDIO/SWDCLK/RESET/GND).
